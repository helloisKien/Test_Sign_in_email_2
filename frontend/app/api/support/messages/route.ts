...
export async function GET(request: Request) {
  try {
    const response = await fetch(`${getServerApiBase(request)}/api/support/messages`, {
      headers: {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = await response.json().catch(() => null);
    return NextResponse.json(payload || { error: "Unable to submit support message." }, { status: response.status });
  } catch {
